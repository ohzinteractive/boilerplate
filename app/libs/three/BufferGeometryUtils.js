...
THREE.BufferGeometryUtils = {
	...
	mergeBufferGeometries: function ( geometries ) {
		...
		for ( var i = 0; i < geometries.length; ++ i ) {
			...
			{
				console.log("BUFFERGEOMETRYUTILS - missing index on indexed geometry")
				return null;
			} 
			...
			for ( var name in geometry.attributes ) {
				...
				{
					console.log("BUFFERGEOMETRYUTILS - missing attribute on one of the geometries: ", name, geometry)
					return null;
				}
		...
		if ( isIndexed ) {
			...
			if ( !mergedIndex ) 
			{
				console.log("BUFFERGEOMETRYUTILS - no mergedIndex")
				return null;
			}
		...
		for ( var name in attributes ) {
			...
			if ( ! mergedAttribute )
			{
				console.log("BUFFERGEOMETRYUTILS - no mergedAttribute")
				return null;
			} 
		...
		for ( var name in morphAttributes ) {
			...
			for ( var i = 0; i < numMorphTargets; ++ i ) {
				...
				{
					console.log("BUFFERGEOMETRYUTILS - no morph attribute");
					return null;
				}
